...
export const uploadMovieImage = (id, image) => async (dispatch) => {
  try {
    ...
    const newhost = host + "/movies/photo/" + id;
...
export const getMovieSuggestion = (id) => async (dispatch) => {
  try {
    const newhost = host + "/movies/usermodeling/" + id;
    const response = await fetch(newhost, {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    });
...
export const updateMovie = (movieId, movie, image) => async (dispatch) => {
  try {
    const token = localStorage.getItem("jwtToken");
    const newhost = host + "/movies/" + movieId;
    const response = await fetch(newhost, {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${token}`,
